...
namespace Procinto.LexicographicalArithmetic
{
	public class LxWord : ILxWord
	{
		...
		{
			if (null == enuc) { Word = null; return this; }

			Word = new List<LxCharacter>(enuc.Count());
			foreach (var ch in enuc) {
				if (ch is LxCharacter) { Word.Add((LxCharacter) ch); } else { /*internal error?*/ }
			}
			if (endian == LxElementOrder.MajorToMinor) { Word.Reverse(); }
			return this;
		}
		...
		{
			if (args == null) { Create(endian, null); return; }
		...
		public bool IsCompatible(ILxWord theOtherWord)
		{
			...
			if (!(theOtherWord is LxWord)) { return false; }

			LxWord that = (LxWord) theOtherWord;
			if (that.Word == null) { return false; }

			// Need to only compare the existing character positions.
			var stopAt = this.Word.Count;
			if (stopAt > that.Word.Count) { stopAt = that.Word.Count; }
		...
		public ILxCharacter this[int i]
		{
			...
			private set { if (IsValid && (i >= 0 || i < Length) && value is LxCharacter) { 
		...
		{
			if (Word == null) { return WEIGHT_INVALID; }
			if (position >= Word.Count) { return WEIGHT_INVALID; }

			if (_weight == null) { CalculateWeights(); }
		...
		{
			if (position < 0) { return WEIGHT_INVALID; }
